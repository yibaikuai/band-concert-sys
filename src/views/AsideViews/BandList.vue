<template>
  <div>
    ...
    >
      ...
      >
        <template #default="scope">
          <span v-show="!scope.row.isEdit"> {{ scope.row.bandNumber }}</span>
          <el-input
            v-show="scope.row.isEdit"
            v-model="scope.row.bandNumber"
            type="number"
            min="1"
            max="99"
      ...
      >
        <template #default="scope">
          <span v-show="!scope.row.isEdit"> {{ scope.row.albumNumber }}</span>
          <el-input
            v-show="scope.row.isEdit"
            v-model="scope.row.albumNumber"
            type="number"
            min="0"
      ...
      >
        <template #default="scope">
          ...
          <el-button
            ...
            type="text"
            size="small"
            @click="cancelEditBandInfo(scope.row)"
          >
            取消编辑
          </el-button>
          <el-button
        ...
        >
          <template #default="scope">
            ...
            <el-button
              ...
              type="text"
              size="small"
              @click="cancelEditMemberInfo(scope.row)"
            >
              取消编辑
            </el-button>
            <el-button
  ...
  methods:{
    ...
    deleteBandInfo(row){
      this.$confirm('此操作将永久删除该乐队, 是否继续?', '提示', {
        ...
        deleteBandApi(row.bandName).then(res=>{
          this.$message({
            type: 'success',
            message: '删除成功!'
          });
    ...
    submitMemberForm() {
      ...
      // console.log(this.addMemberInfo)
    ...
    getBandData(){
      getList(this.currentPage,10,this.type).then((res)=>{
        let {data} = res.data
